...
seurat.tum = subset(seurat, cells=which(seurat$Tissue=="Tum")) #keep only cells from tumoral samples
...
#BARPLOT
my.family=c("Growth factor","Chemokine","Checkpoint","Cytokine","Notch family","Antigen binding", "Cell adhesion")
family.col = c( "Growth factor"= "#AECBE3", "Chemokine"= "#66ABDF", "Checkpoint"= "#1D1D18"  , "Cell adhesion"="slateblue4",
...
p1
ggsave(paste0(output.dir,"ICELLNET_", assay,"/", CC1,"_", direction, "_barplot_",Sys.Date(),".pdf")) # Figure 3B-left

p2
ggsave(paste0(output.dir,"ICELLNET_", assay,"/", CC2,"_", direction, "_barplot_",Sys.Date(),".pdf")) # Figure 3B-right
...
#-----------------------------------------------
# Contribution ccRCC2  (for each interaction) compared to other cells - analysis considering all the db (not only interactions >0 for TumC)
#========================
...
head(LR.mat)
LR.mat=LR.mat[,-c(1,24)] # do not forget to remove X, and ccRCC1 -> to study ccRCC2 specificity

test=int.spe(LR.mat, CoI="ccRCC2", thresh=thresh)
...
### 2nd step # ADD info of DEG -> keep only interaction where ccRCC cells' gene is upregulated compared to PT
DEG=as.data.frame(read.delim("analyses/3_Downstream_analysis/DEG_TumC_RNA/DEG_ccRCC2_CA9_Tum_vs_PT_Hty_inDB_logFC0.1_minpct0.05_padj0.05.csv", sep = ",", header = T))
...
write.csv(test2, paste0(work.dir,output.dir,'ICELLNET_', assay, "/LR_spé_tumC_DEG",direction,"_thresh_",thresh,"_ALL_CELLS_", Sys.Date(),".csv")) #Listed in Supplementary Table S4
